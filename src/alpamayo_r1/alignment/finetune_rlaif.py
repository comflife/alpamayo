#!/usr/bin/env python3
"""
RLAIF (Reinforcement Learning from AI Feedback) for Alpamayo-R1.

════════════════════════════════════════════════════════════════════════════════
              🎯 RESEARCH CONTRIBUTION: Learning from LLM Feedback
════════════════════════════════════════════════════════════════════════════════

PROBLEM: GT labels may have poor quality in BOTH reasoning and trajectory.
Traditional SFT blindly mimics GT even when reasoning doesn't match trajectory.

SOLUTION: Use LLM (Claude) as a judge to evaluate trajectory + reasoning quality
based on detailed off-road driving guidelines.

════════════════════════════════════════════════════════════════════════════════
                              🧠 CORE INNOVATIONS
════════════════════════════════════════════════════════════════════════════════

1. **LLM Judge with Guidelines**
   ------------------------------
   Claude API evaluates each (reasoning, trajectory) pair:

   - Trajectory Quality (0-10): Does it follow tire tracks? Avoid puddles/mud?
   - Reasoning Quality (0-10): Does it describe the scene accurately?
   - Consistency (0-10): Do reasoning and trajectory match?

   Guidelines include:
   - Drivable path identification
   - Puddle/water avoidance
   - Rut management
   - Safe clearance from edges

2. **Multi-Candidate Sampling**
   ------------------------------
   For each training sample:
   1. Generate N (reasoning, trajectory) candidates with varied temperature
   2. Visualize each candidate on the camera image
   3. Send to LLM judge with guidelines
   4. Get detailed scores and feedback

3. **Reward from AI Feedback**
   ----------------------------
   Reward = (trajectory_quality + reasoning_quality + consistency) / 3

   This replaces hand-crafted visual safety scoring with LLM judgment

4. **Policy Gradient with LLM Rewards**
   -------------------------------------
   For each batch:
   1. Sample N trajectory+reasoning pairs
   2. Get LLM evaluation for each
   3. Compute advantage: A = R - baseline_reward
   4. Update policy: Maximize log_prob of high-reward samples

   Language and trajectory are trained jointly to maximize LLM feedback
...
✓ Generate trajectories that follow detailed off-road guidelines
✓ Produce reasoning that accurately describes the scene
✓ Maintain consistency between reasoning and trajectory
✓ Improve beyond GT quality through LLM feedback
...
torchrun --nproc_per_node=2 -m alpamayo_r1.alignment.finetune_rlaif \
...
Key hyperparameters:
--------------------
--num_trajectory_samples 4       # Number of candidates to generate per sample
--rlaif_loss_weight 1.0          # Weight for RLAIF loss (vs SFT)
--language_loss_weight 0.0       # Set to 0 to disable GT language loss
--traj_loss_weight 0.0           # Set to 0 to rely purely on RLAIF
--judge_model "claude-3-5-sonnet-20241022"  # LLM judge model
--sampling_temperature 0.8       # Temperature for candidate generation

Full RLAIF mode (no GT supervision):
-------------------------------------
--language_loss_weight 0.0 --traj_loss_weight 0.0 --rlaif_loss_weight 1.0

Hybrid mode (RLAIF + SFT):
--------------------------
--language_loss_weight 0.5 --traj_loss_weight 0.5 --rlaif_loss_weight 1.0
